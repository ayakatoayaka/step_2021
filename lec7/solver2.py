...
# def translocation_mutation(score_list,genes, num_mutation, p_value):
#     mutated_genes = []
#     not_mutated_genes = []
#     true_or_false = False
#     for gene in genes:
#         for _,sl in score_list:
#             if sl == gene:
#                 mutated_genes.append(gene)
#                 true_or_false = True
#         if true_or_false==False:
#             not_mutated_genes.append(gene)
...
#             for k in range(len(genes[i])):
#                 if genes[i][k] == mutation_value[0] and mutation_value[0] in not_mutated_genes:
#                     mutation_position1 = k
#                     break
#             for k in range(len(genes[i])):
#                 if genes[i][k] == mutation_value[1] and mutation_value[0] in not_mutated_genes:
#                     mutation_position2 = k
#                     break
#             # mutation_position2 = np.where(genes[i] == mutation_value[1])
#             mutated_genes[i][mutation_position1] = mutation_value[1]
#             mutated_genes[i][mutation_position2] = mutation_value[0]
#     return mutated_genes+not_mutated_genes
...
def solve(cities):
    ...
    for _,index in score_list:
        i+=1
        if i<=50:
            remain_path_list.append(path_list[index])
        else:
            new_path_list.append(path_list[index])
    ...
    for j in range(5000):
        ...
        crossovered_path_list = crossover(new_path_list)
        ...
        new_path_list = translocation_mutation(crossovered_path_list,500,0.3)
        path_list = new_path_list+remain_path_list
        score_list = make_score_list(path_list, dist)
